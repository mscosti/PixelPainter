...
let loadImage = input => {
  ...
  function imageLoaded () {
    ...
    ctx.setTransform(
      0,
      1, // x axis down the screen
      -1,
      0, // y axis across the screen from right to left
    ...
    // ctx.restore()
    window.fullDataStr = genBitmapImage(
...
var genBitmapImage = function (oData) {
  ...
  var entire_uint8_data = new Uint8Array(
    BITMAPFILEHEADER.concat(BITMAPINFOHEADER).concat(pixel_array)
  );
...
var run = () => {
  ...
  let loop_image = document.getElementById('loop');
  ...
  let period = document.getElementById('periodInputId');
  let dutyCycle = document.getElementById('dutyCycleInputId');
  let blend = document.getElementById('blend');
  let animate = document.getElementById('animate');

  getSelectedColors = () => {
    return [...colorsList.children].map(row => {
      return hexToRgb(row.querySelector('input').value);
    });
  };
  ...
  start.addEventListener('click', e => {
    let data = {
      loop_image: loop_image.checked
    };
    console.log(data);
    fetch('/ajax/startImage', {
      method: 'POST',
      mode: 'no-cors',
  ...
  addColor.addEventListener('click', e => {
    ...
    var colorItem = document.createElement('input');
  ...
  startColor.addEventListener('click', e => {
    console.log('sending colors');
    let data = {
      colors: getSelectedColors(),
      period: period.valueAsNumber,
      duty_cycle: dutyCycle.valueAsNumber,
      blend: blend.checked,
      animate: animate.checked
    };
    console.log(data);
    fetch('/ajax/startColors', {
      method: 'POST',
      mode: 'no-cors',
      body: JSON.stringify(data)
    });
...
function formatData (ctx, width, height) {
  ...
  ctx
    ...
    .data.forEach(val => big_arr.push(val));
